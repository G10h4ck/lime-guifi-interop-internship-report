...
- Blue curvy lines: WiFi mesh links.
...
Guifi.net BGP routers are form different vendors and often they have different implementation on wich route attributes are considered when deciding wich path to prefere, after reading documentations from different vendors, seems to me that in eBGP setups (like Guifi.net) the most realiable way to share a route with low preference is to artificially enlarge it's AS path, this tecnique is called AS-path-prepending and it's common in situation where someone want to share a route but want others routers prefere other paths if there is some path available.
some references:
