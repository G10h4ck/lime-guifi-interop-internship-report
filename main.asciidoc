...
- NAT based network are unfair because the _natted_ host lose the opportunity to be a server (listening for incoming connections), and the non _natted_ automatically acquire the privilege of listening for connections that isn't fairly distributed to all hosts anymore.
...
// Breaking page here do stuff even worser.
